...
<body>
    ...
    <%
        String id = request.getParameter("id");
        Locacao locacao = LocacaoDao.getLocacaoById(Integer.parseInt(id));
        Cliente cliente = ClienteDao.getClienteById(locacao.getIdCliente());
        Veiculo veiculo = VeiculoDao.getVeiculoById(locacao.getIdVeiculo());
    %>
    <div class="container">
        <h1>
            Editar veículo:
        ...
        <div>
            ...
            <a href="cadastro-veiculo.jsp"> Cadastro Locação </a> 
            <a href="consulta-veiculo.jsp"> Consulta Locação </a>
        </div>

<form id="fcad" action="cadastro-locacao.jsp" method="POST">

 <input type="hidden" id="id" name="id" value="<%out.print(locacao.getId());%>" />

        <label for="placa">Placa do veículo</label>
        <input type="text" id="placa" name="placa" placeholder="EX: ABC-1234" onblur="verificarNome()" value="<%out.print(veiculo.getPlaca());%>"/>

        <label for="nomeCliente">Nome do cliente</label>
        <input type="text" id="nomeCliente" name="nomeCliente" placeholder="EX: João da Silva" value="<%out.print(cliente.getNome());%>"/>

        <label for="dataInicial">Data inicial da locação</label>
        <input type="date" id="dataInicial" name="dataInicial" value="<%out.print(locacao.getDataInicio());%>" />

        <label for="dataFim">Data final da locação</label>
        <input type="date" id="dataFim" name="dataFim" value="<%out.print(locacao.getDataFim());%>"/>
